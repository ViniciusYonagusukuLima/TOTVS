...
// Para ativar essa função: Abro a Base de dados, incluo uma nova tabela e preencho o nome da função no campo.
Function U_TOOLS001

    // Referência da Static Function abaixo
    TOOLS001('Z50',.F.)
    TOOLS001('Z51',.F.)
	TOOLS001('Z52',.F.)
	TOOLS001('Z53',.F.)

    // Após abrir o browser no Protheus
    fwAlertInfo('Ok')
...
Static Function TOOLS001(cTabela,lShowMsg)
    ...
    // class_sxs() é a classe padrão para manipular tabelas SX2, SX3, SIX, SXB.
    // :new() é o construtor dessa classe ou seja, inicializa o objeto.
    Private oTools  := class_sxs():new()

    // Bons modos é instanciar o Default dos parâmetros da função.
    Default cTabela := ''
    Default lShowMsg:= .F.

    // empty() Verifica se a variável está vazia
    IF empty(cTabela)
        // Caso for verdadeiro, return faz a função parar e sair imediatamente
        return
    EndIF    

    // fwMsgRun() executa uma função, enquanto isso exibe uma mensagem.
    // oemtoansi() converte a string.
...
// Finaliza a função e passa para próxima linha.
Return
